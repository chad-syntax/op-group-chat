import { AgentsmithClient, Message } from '@agentsmith-app/sdk';
...
const main = async () => {
  try {
    const prompt = await client.getPrompt('hello-world');
    ...
    const callLLM = async (messages: Message[]) => {
      ...
      const { compiledPrompt, completion } = res;
      ...
      if (messages.length === 0) {
        ...
        messages.push((await completion).choices[0].message);
      } else {
        messages.push((await completion).choices[0].message);
      }
